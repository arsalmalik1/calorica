...
class DBMealProductHistoryProvider {
  DBMealProductHistoryProvider._();

  static final DBMealProductHistoryProvider db =
      DBMealProductHistoryProvider._();
  ...
  initDB() async {
    ...
    String path = join(documentsDirectory.path, "MealProductsHistory.db");
    return await openDatabase(path, version: 1, onOpen: (db) {},
        onCreate: (Database db, int version) async {
      await db.execute("CREATE TABLE MealProductsHistory ("
  ...
  Future<int> addProduct(WishlistMealProduct product) async {
    ...
    var table =
        await db.rawQuery("SELECT MAX(id)+1 as id FROM MealProductsHistory");
    int id = table.first["id"];
    var raw = await db.rawInsert(
        "INSERT Into MealProductsHistory (id, name, category, calory, squi, fat, carboh, gram, meal_id, product_id)"
        " VALUES (?,?,?,?,?,?,?,?,?,?)",
        [
          id,
          product.name,
  ...
  Future<List<Product>> getAllProductsSearch(String text) async {
    final db = await database;
    var res = await db.query("MealProductsHistory",
  ...
  Future<List<WishlistMealProduct>> getProductByMealID(int id) async {
    ...
    var res = await db
        .rawQuery("SELECT * FROM MealProductsHistory WHERE meal_id = $id");
    for (var item in res) {
  ...
  Future<List<WishlistMealProduct>> getProducts() async {
    final db = await database;
    List<WishlistMealProduct> products = [];
    var res = await db.rawQuery("SELECT * FROM MealProductsHistory");
    for (var item in res) {
      WishlistMealProduct product = WishlistMealProduct(
          id: item["id"],
          name: item["name"],
  ...
  Future deleteProductsByMeal(id) async {
    final db = await database;
    var res = await db
        .rawQuery("DELETE FROM MealProductsHistory WHERE meal_id = $id");
    print(res);
    return res;
  }
  ...
  Future deleteAllWishlistMealProducts() async {
    final db = await database;
    var res = await db.rawQuery("DELETE FROM MealProductsHistory");
    print(res);
    return res;
  }
  ...
  Future deleteAllMealProducts(int mealID) async {
    ...
    var list = [mealID];
    return await db.delete('MealProductsHistory',
  ...
  Future<int> updateProduct(WishlistMealProduct product) async {
    ...
    final count = await db.rawUpdate(
        'UPDATE MealProductsHistory SET name = ?, category = ?, calory = ?, squi = ?, fat = ?, carboh = ?, gram = ?, meal_id = ?, product_id = ? WHERE id = ?',
        [
          '${product.name}',
          '${product.category}',
          '${product.calory}',
